"""SOAP 1.1 envelope parsing and response/fault construction."""
...
def parse_request(xml_bytes: bytes) -> tuple[str, ET.Element, ET.Element]:
    ...
    if len(operations) != 1:
        raise ValueError("El SOAP Body debe contener una sola operación.")
    ...
    return (
        operation.tag.rsplit("}", 1)[-1],
        header if header is not None else ET.Element("Header"),
        operation,
    )
...
def fault(code: str, message: str, detail: str | None = None) -> bytes:
    root = ET.Element(f"{{{SOAP_NS}}}Envelope")
    body = ET.SubElement(root, f"{{{SOAP_NS}}}Body")
    node = ET.SubElement(body, f"{{{SOAP_NS}}}Fault")
    ET.SubElement(node, "faultcode").text = code
    ET.SubElement(node, "faultstring").text = message
    if detail:
        ET.SubElement(node, "detail").text = detail
    return ET.tostring(root, encoding="utf-8", xml_declaration=True)
